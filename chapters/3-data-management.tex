...
\begin{itemize}
    ...
    \item \textbf{Wavelet transform}. % experimental, Hilbert curve, space-filling curve needed to order the data points to be local to each other?
    
    \item \textbf{Transparent}. % helper, for testing, or for grouping of timesteps into one file z nejakeho duvodu

\end{itemize}

% polynomial functions - jde mimo pouzity format, je to alternativa k pouzitemu reseni. byla navrzena jeste pred vznikem tohoto formatu. Myslenka je provadet kompresi tak, ze nahradim diskretni data spojitymi polynomialnimi funkcemi urciteho radu, zminuji to zde jen pro uplnost, podrobnosti jsou v kapitole \ref{chapter:approximation}


\subsection {Encoding}
\label{subsec:encoding}
% Encoding is a process of transforming the data ...
% encodingem se mysli sekvence kroku: nejprve analyza datovych poli, zda neobsahuje repetice a sla by tedy zkratit, ci nahradit jednou hodnotou, protoze datova pole absohujici napr same nuly, jsou pomerne bezna - zakoduje se pomoci defaultvalue a hodnota length muze byt nula. originallength urcuje puvodni delku pole. Take jsou bezna pole obsahujici hodne hodnot typu NaN - chybejici data v dane lokaci - to se zakoduje pomoci vlastnosti offset a length.
% dalsim krokem je prevedeni z binarni podoby do textove - na to se pouziva base64 kodovani
% dalsim krokem je serializace do transportniho formatu JSON (nebo napr XML) pomoci ktereho se data transportuji bud do db, nebo do vzdaleneho blob storage, nebo ulozi na lokalni disk.

%Properties DataType, OriginalLength, Offset, Length, DefaultValue; DataType possible values: "Float64", "Float32", "Int32", "UInt8"

% Dat sem nacrtek celeho procesu konverze do mojeho formatu? data -> komprese -> kodovani -> storage -> dekodovani -> castecna dekomprese jednoho radku; oznacit co se deje ve format converteru, a co se deje v post procesoru. Asi ne.
...
% layers, filters, vytvoreni Surface vrstvy pro webovy postprocessor, barevna skala, prepinani skalarnich velicin, vektorove veliciny - je treba nacist vice komponent najednou (to je jedine, kdy je to potreba). Dekomprese: prenasobeni matic - staci jeden radek. ...
...
% TODO: do results class diagramu pridat pole FallBackLayerId a pak ho v textu popsat: Summary document muze take volitelne obsahovat property MeshFallbackLayerId, AttributeFallbackLayerId, a DataFallbackLayerId - vyuzito napr pro deformacni vrstvu - nemusi se kopirovat data
