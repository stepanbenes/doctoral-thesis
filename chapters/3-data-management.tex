...
% TODO: move the following into the next section
...
% The specification of the format follows:
...
As can be seen in the example of the solution document in Listing \ref{lst:solution.json}, each layer is assigned a unique identifier in form of GUID\footnote{TODO: GUID...} number. 
% Solution obsahuje vrstvy - kazda ma unikatni id, ktere zaroven koduje umisteni souboru dane vrstvy.

Data in each layer are stored in four types of documents.

\begin{itemize}
    \item \textbf{summary document} --- ... Listing \ref{lst:summary.json}
    \item \textbf{mesh document} --- ... Listing \ref{lst:mesh.json}
    \item \textbf{attribute document} --- ... Listing \ref{lst:attribute.json}
    \item \textbf{result document} --- ... Listing \ref{lst:result.json}
\end{itemize}
...
% shrnout proc je to vyhodne mit to takto rozdelene do jednotlivych souboru. vypada to divoce a slozite, proc je to lepsi nez mit to v jednom souboru?
