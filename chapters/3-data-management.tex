...
\begin{itemize}
    ...
    \item \textbf{Attribute selection} -- % Attribute selection filter allows to choose mesh entities (usually elements) having a particular attribute assigned and propagate them into a new layer, filtering out the other entities, that does not have the attribute assigned. It is useful for material property ... Figure \ref{fig:beam-attribute-selection-layer} shows an example of a layer 
    ...
    \item \textbf{Surface} -- 
    % Surface, // surface of the mesh, dulezite pro webovy post-procesorm ktery umi jen trojuhelniky kvuli jednoduchosti - TODO: pridat obrazek povrchu beamu (upozornit na trojuhelniky misto krychli)
    % Helper filter; attribute muze obsahovat cislovani a puvodni plochy prvku pak mohou byt rekonstruovany; pripadne se daji vygenerovat quady, ale trojuhelniky jsou nejjednodussi pro implementaci jedoducheho postprocesoru
...
% TODO: postprocesoru ma zobrazeny strom vrstev a muze jednotlive vrstvy vypinat ci zapinat a menit u nich zobrazovaci mod - outlines/wireframe/solid; po pridani nove vrstvy (napr. deformace) se typicky rodicovska prepne do modu outlines aby byl videt vztah puvodni a odvozene vrstvy.

% TODO: obrazek obou post-procesoru? Nebo az do kapitoly Overall results?

% TODO: barevna skala, prepinani skalarnich velicin?, vektorove veliciny - je treba nacist vice komponent najednou (to je jedine, kdy je to potreba). Dekomprese: prenasobeni matic - staci jeden radek. ... Jak probiha cely proces dotazu a zobrazeni dat ve webovem postprocesoru.
...
% TODO: isoareas pomoci shaderu - pridat obrazek. neni to ve storage formatu, negeneruje se pro to jina geometrie, vse je elegantne zarizeno pomoci shaderu do ktereho je zakodovana barevna skala sestavajici z kontrolnich bodu...
