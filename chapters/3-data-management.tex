...
The prototype implementation contains the following types of compression methods:

\begin{itemize}
    \item \textbf{SVD} -- promising, property \code{Rank}
    % main features for optimization: key time steps (time step span compression), Randomized SVD, Parallelization, Sparse matrix of details, prenasobeni U matice singularnimi cisly, trochu usetrim pamet, mohu pouzit vzorkovani... % See chapter \ref{chapter:SVD}

    \item \textbf{Wavelet transformation} -- experimental, Hilbert curve?
    \item \textbf{Transparent} -- helper
\end{itemize}

% polynomial functions - jde mimo pouzity format, zminuji to zde jen pro uplnost, podrobnosti jsou v kapitole \ref{chapter:approximation}


\subsection {Encoding}
\label{subsec:encoding}

% encodinge se mysli sekvence kroku: nejprve analyza datovych poli, zda neobsahuje repetice a sla by tedy zkratit, ci nahradit jednou hodnotou, protoze datova pole absohujici napr same nuly, jsou pomerne bezna - zakoduje se pomoci defaultvalue a hodnota length muze byt nula. originallength urcuje puvodni delku pole. Take jsou bezna pole obsahujici hodne hodnot typu NaN - chybejici data v dane lokaci - to se zakoduje pomoci vlastnosti offset a length.
